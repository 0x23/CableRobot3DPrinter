...
#include "arm_solutions/DeltaStriderSolution.h"
...
#define  deltastrider_checksum               CHECKSUM("deltastrider")
...
void Robot::on_config_reload(void *argument)
{
    ...
    } else if(solution_checksum == deltastrider_checksum) {
        this->arm_solution = new DeltaStriderSolution(THEKERNEL->config);
...
void Robot::on_gcode_received(void *argument)
{
    ...
    if( gcode->has_g) {
        ...
        switch( gcode->m ) {
			...
			// DeltaStrider
			case 666: { // M666 directly move Acrtuator a given number of millimeters
				int actuator = gcode->has_letter('A') ? gcode->get_value('A') : -1;
				float dist = gcode->has_letter('D') ? gcode->get_value('D') : 0.0f;
				if(actuator<0 || actuator>=3)
					break;
			
                gcode->stream->printf("moving actuator %i by %fmm (Motors must be enabled)\n", actuator, dist);

				float StepsPerMM = THEKERNEL->robot->actuators[actuator]->get_steps_per_mm();
				int Steps = dist*StepsPerMM;
				THEKERNEL->robot->actuators[actuator]->move(Steps<0, abs(Steps), 10.0f*StepsPerMM);
				
				gcode->stream->printf("WARNING!: THE CALIBRATION AND ARM SOLUTION IS NOW INVALID!\n");
			}
        }
    }

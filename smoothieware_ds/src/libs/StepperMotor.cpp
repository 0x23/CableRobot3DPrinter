...
// or stops the motor if steps is 0
// ### modified DeltaStrider: motor stops when steps == 0
// after each to call to move a speed value should be set otherwise the motor will stop because fx_ticks_per_step is set to a high value
StepperMotor* StepperMotor::move( bool direction, unsigned int steps, float initial_speed )
{
	...
	// backup old velocity (DeltaStrider)
	// uint32_t old_fx_ticks_per_step = fx_ticks_per_step;
...
//		else // restore velocity (DeltaStrider)
//			fx_ticks_per_step = old_fx_ticks_per_step;
        this->moving = true;
    } else {
		// emit signal if motor was moving and someone stopped it by calling move with 0 steps

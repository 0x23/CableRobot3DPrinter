...
// returns the config value as a list of numbers (example config: my_value = 1.2, 2.3, 6.9, 4.5)
vector<float> ConfigValue::as_number_list()
{
    ...
    if( this->found == false && this->default_set == true ) {
        ...
        do {
            ...
            std::string element = value.substr(old_pos, pos-old_pos);
            
            // convert to number
            char *endptr = NULL;
            string str = remove_non_number(element);
            const char *cp= str.c_str();
            float result = strtof(cp, &endptr);
            if( endptr <= cp )
                printErrorandExit("config setting with value '%s' and checksums[%04X,%04X,%04X] is not a valid number list, please see http://smoothieware.org/configuring-smoothie\r\n", this->value.c_str(), this->check_sums[0], this->check_sums[1], this->check_sums[2] );     

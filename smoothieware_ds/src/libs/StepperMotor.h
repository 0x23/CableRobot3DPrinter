...
class StepperMotor {
    public:
        ...
        // added DeltaStrider, check if this is correct for fixed point step counter
		float get_stepped_accurate() const { return float(stepped) + float(fx_counter)/float(fx_ticks_per_step); }
